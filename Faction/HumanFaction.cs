...
// Uses HumanTech for all unit stats - no hardcoded values!
...
namespace TheWaningBorder.Gameplay
{
    ...
    public static class HumanFaction
    {
        ...
        /// CRITICAL: HumanTech.Load() must be called BEFORE this!
        /// </summary>
        ...
        /// Spawns each player with Hall + army using HumanTech stats.
        /// </summary>
        public static void GeneratePlayers(EntityManager em, int playerCount)
        {
            if (playerCount <= 0) return;

            // CRITICAL: Verify HumanTech is loaded
        ...
        /// All units get stats from HumanTech.
        /// </summary>
        static void SpawnPlayerStart(EntityManager em, float3 spawnPos, Faction fac)
        {
            ...
            Debug.Log("esta merda devia gerar soldadinhos");
            ...
            Builder.Create(em, builderPos, fac);
        ...
        /// <summary>
        /// Spawn army in formation with stats from HumanTech.
        ...
        static void SpawnArmyWithStats(
            ...
            int archersCount)
        {
            swordsmenCount = math.max(0, swordsmenCount);
            archersCount = math.max(0, archersCount);
            int totalUnits = swordsmenCount + archersCount;
            if (totalUnits == 0) return;

            int cols = (int)math.ceil(math.sqrt(totalUnits));
            int rows = (totalUnits + cols - 1) / cols;
            float spacing = 1.5f;
            ...
            float3 topLeft = centerPos - right * (width * 0.5f) - fwd * (height * 0.5f);

            for (int i = 0; i < totalUnits; i++)
            {
                int row = i / cols;
                int col = i % cols;
                float3 pos = topLeft + right * (col * spacing) + fwd * (row * spacing);

                if (i < swordsmenCount)
                    Swordsman.Create(em, pos, faction);
                else
                    Archer.Create(em, pos, faction);
            }
        }
    }
}