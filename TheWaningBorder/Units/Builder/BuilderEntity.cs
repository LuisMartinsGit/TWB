...
namespace TheWaningBorder.Units.Builder
{
    ...
    public class BuilderEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Builder
            builderArchetype = EntityManager.CreateArchetype(
        ...
        public Entity CreateBuilder(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Builder");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Builder data not found in TechTree.json!");
                throw new InvalidOperationException("Builder configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(builderArchetype);
