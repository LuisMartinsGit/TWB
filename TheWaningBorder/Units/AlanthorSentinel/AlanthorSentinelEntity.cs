...
namespace TheWaningBorder.Units.AlanthorSentinel
{
    ...
    public partial class AlanthorSentinelEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Alanthor_Sentinel
            alanthorsentinelArchetype = EntityManager.CreateArchetype(
                ...
                typeof(AlanthorSentinelTag)
            );
        }

        public Entity CreateAlanthorSentinel(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Alanthor_Sentinel");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Alanthor_Sentinel data not found in TechTree.json!");
                throw new InvalidOperationException("Alanthor_Sentinel configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(alanthorsentinelArchetype);
