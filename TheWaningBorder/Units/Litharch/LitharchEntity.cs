...
{
    /// <summary>
    /// Entity definition for Litharch unit
    /// All values MUST be loaded from TechTree.json - NO HARDCODED VALUES!
    /// </summary>
    public partial class LitharchEntity : DataLoaderSystem
    {
        private EntityArchetype litharchArchetype;

        protected override void OnCreate()
        {
            base.OnCreate();
            
            // Create archetype for Litharch
            litharchArchetype = EntityManager.CreateArchetype(
        ...
        public Entity CreateLitharch(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Litharch");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Litharch data not found in TechTree.json!");
                throw new InvalidOperationException("Litharch configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(litharchArchetype);
