...
namespace TheWaningBorder.Units.Archer
{
    ...
    public class ArcherEntity : DataLoaderSystem
    {
        ...
        public Entity CreateArcher(float3 spawnPosition)
        {
            ...
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Archer data not found in TechTree.json!");
                throw new InvalidOperationException("Archer configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(archerArchetype);
