...
{
    /// <summary>
    /// Entity definition for Runai_Raider unit
    /// All values MUST be loaded from TechTree.json - NO HARDCODED VALUES!
    /// </summary>
    public partial class RunaiRaiderEntity : DataLoaderSystem
    {
        private EntityArchetype runairaiderArchetype;

        protected override void OnCreate()
        {
            base.OnCreate();
            
            // Create archetype for Runai_Raider
            runairaiderArchetype = EntityManager.CreateArchetype(
                ...
                typeof(RunaiRaiderTag)
            );
        }

        public Entity CreateRunaiRaider(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Runai_Raider");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Runai_Raider data not found in TechTree.json!");
                throw new InvalidOperationException("Runai_Raider configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(runairaiderArchetype);
