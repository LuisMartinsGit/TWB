...
namespace TheWaningBorder.Units.FeraldisSiegeRam
{
    ...
    public partial class FeraldisSiegeRamEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Feraldis_SiegeRam
            feraldissiegeramArchetype = EntityManager.CreateArchetype(
                ...
                typeof(FeraldisSiegeRamTag)
            );
        }

        public Entity CreateFeraldisSiegeRam(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Feraldis_SiegeRam");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Feraldis_SiegeRam data not found in TechTree.json!");
                throw new InvalidOperationException("Feraldis_SiegeRam configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(feraldissiegeramArchetype);
