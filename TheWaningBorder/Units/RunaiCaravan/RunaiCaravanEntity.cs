...
namespace TheWaningBorder.Units.RunaiCaravan
{
    ...
    public class RunaiCaravanEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Runai_Caravan
            runaicaravanArchetype = EntityManager.CreateArchetype(
                ...
                typeof(RunaiCaravanTag)
            );
        }

        public Entity CreateRunaiCaravan(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Runai_Caravan");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Runai_Caravan data not found in TechTree.json!");
                throw new InvalidOperationException("Runai_Caravan configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(runaicaravanArchetype);
