...
namespace TheWaningBorder.Units.AlanthorBallista
{
    ...
    public class AlanthorBallistaEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Alanthor_Ballista
            alanthorballistaArchetype = EntityManager.CreateArchetype(
                ...
                typeof(AlanthorBallistaTag)
            );
        }

        public Entity CreateAlanthorBallista(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Alanthor_Ballista");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Alanthor_Ballista data not found in TechTree.json!");
                throw new InvalidOperationException("Alanthor_Ballista configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(alanthorballistaArchetype);
