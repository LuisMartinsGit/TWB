...
{
    /// <summary>
    /// Entity definition for Feraldis_WarboarRider unit
    /// All values MUST be loaded from TechTree.json - NO HARDCODED VALUES!
    /// </summary>
    public partial class FeraldisWarboarRiderEntity : DataLoaderSystem
    {
        private EntityArchetype feraldiswarboarriderArchetype;

        protected override void OnCreate()
        {
            base.OnCreate();
            
            // Create archetype for Feraldis_WarboarRider
            feraldiswarboarriderArchetype = EntityManager.CreateArchetype(
                ...
                typeof(FeraldisWarboarRiderTag)
            );
        }

        public Entity CreateFeraldisWarboarRider(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Feraldis_WarboarRider");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Feraldis_WarboarRider data not found in TechTree.json!");
                throw new InvalidOperationException("Feraldis_WarboarRider configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(feraldiswarboarriderArchetype);
