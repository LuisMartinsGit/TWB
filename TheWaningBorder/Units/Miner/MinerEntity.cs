...
namespace TheWaningBorder.Units.Miner
{
    ...
    public partial class MinerEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Miner
            minerArchetype = EntityManager.CreateArchetype(
        ...
        public Entity CreateMiner(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Miner");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Miner data not found in TechTree.json!");
                throw new InvalidOperationException("Miner configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(minerArchetype);
