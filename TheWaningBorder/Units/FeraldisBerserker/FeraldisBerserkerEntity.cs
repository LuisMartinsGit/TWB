...
namespace TheWaningBorder.Units.FeraldisBerserker
{
    ...
    public partial class FeraldisBerserkerEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Feraldis_Berserker
            feraldisberserkerArchetype = EntityManager.CreateArchetype(
                ...
                typeof(FeraldisBerserkerTag)
            );
        }

        public Entity CreateFeraldisBerserker(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Feraldis_Berserker");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Feraldis_Berserker data not found in TechTree.json!");
                throw new InvalidOperationException("Feraldis_Berserker configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(feraldisberserkerArchetype);
