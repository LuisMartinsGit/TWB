...
namespace TheWaningBorder.Units.Swordsman
{
    ...
    public partial class SwordsmanEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Swordsman
            swordsmanArchetype = EntityManager.CreateArchetype(
        ...
        public Entity CreateSwordsman(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Swordsman");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Swordsman data not found in TechTree.json!");
                throw new InvalidOperationException("Swordsman configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(swordsmanArchetype);
