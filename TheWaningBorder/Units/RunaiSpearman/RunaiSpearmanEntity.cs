...
namespace TheWaningBorder.Units.RunaiSpearman
{
    ...
    public partial class RunaiSpearmanEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Runai_Spearman
            runaispearmanArchetype = EntityManager.CreateArchetype(
                ...
                typeof(RunaiSpearmanTag)
            );
        }

        public Entity CreateRunaiSpearman(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Runai_Spearman");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Runai_Spearman data not found in TechTree.json!");
                throw new InvalidOperationException("Runai_Spearman configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(runaispearmanArchetype);
