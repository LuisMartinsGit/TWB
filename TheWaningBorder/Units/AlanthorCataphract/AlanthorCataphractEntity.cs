...
{
    /// <summary>
    /// Entity definition for Alanthor_Cataphract unit
    /// All values MUST be loaded from TechTree.json - NO HARDCODED VALUES!
    /// </summary>
    public partial class AlanthorCataphractEntity : DataLoaderSystem
    {
        private EntityArchetype alanthorcataphractArchetype;

        protected override void OnCreate()
        {
            base.OnCreate();
            
            // Create archetype for Alanthor_Cataphract
            alanthorcataphractArchetype = EntityManager.CreateArchetype(
                ...
                typeof(AlanthorCataphractTag)
            );
        }

        public Entity CreateAlanthorCataphract(float3 spawnPosition)
        {
            // Load unit data from TechTree.json
            var unitData = GetUnitData("Alanthor_Cataphract");
            
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Alanthor_Cataphract data not found in TechTree.json!");
                throw new InvalidOperationException("Alanthor_Cataphract configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(alanthorcataphractArchetype);
