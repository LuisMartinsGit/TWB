...
{
    /// <summary>
    /// Entity definition for Runai_Skirmisher unit
    /// All values MUST be loaded from TechTree.json - NO HARDCODED VALUES!
    /// </summary>
    public partial class RunaiSkirmisherEntity : DataLoaderSystem
    {
        private EntityArchetype runaiskirmisherArchetype;

        protected override void OnCreate()
        {
            base.OnCreate();
            
            // Create archetype for Runai_Skirmisher
            runaiskirmisherArchetype = EntityManager.CreateArchetype(
                ...
                typeof(RunaiSkirmisherTag)
            );
        }

        public Entity CreateRunaiSkirmisher(float3 spawnPosition)
        {
            ...
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Runai_Skirmisher data not found in TechTree.json!");
                throw new InvalidOperationException("Runai_Skirmisher configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(runaiskirmisherArchetype);
