...
namespace TheWaningBorder.Units.FeraldisHunter
{
    ...
    public partial class FeraldisHunterEntity : DataLoaderSystem
    {
        ...
        protected override void OnCreate()
        {
            ...
            // Create archetype for Feraldis_Hunter
            feraldishunterArchetype = EntityManager.CreateArchetype(
                ...
                typeof(FeraldisHunterTag)
            );
        }

        public Entity CreateFeraldisHunter(float3 spawnPosition)
        {
            ...
            if (unitData == null)
            {
                Debug.LogError("CRITICAL ERROR: Feraldis_Hunter data not found in TechTree.json!");
                throw new InvalidOperationException("Feraldis_Hunter configuration missing from TechTree.json!");
            }

            // Create entity with loaded data
            var entity = EntityManager.CreateEntity(feraldishunterArchetype);
