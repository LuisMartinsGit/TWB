// FILE: Assets/Scripts/AI/AIBuildingManager.cs
...
namespace TheWaningBorder.AI
{
    ...
    public partial struct AIBuildingManager : ISystem
    {
        ...
        private int CountQueuedBuilders(ref SystemState state, Faction faction)
        {
            ...
            // Check recruitment requests for builders
            foreach (var (brain, recruitReqs) in SystemAPI.Query<RefRO<AIBrain>, DynamicBuffer<RecruitmentRequest>>())
            {
                ...
                for (int i = 0; i < recruitReqs.Length; i++)
                {
                    ...
                    if (req.UnitType == UnitClass.Economy) // Builders are economy units
                    {
                        count += req.Quantity;
                    }
                }
            ...
            // Check training queues at barracks/halls for builders
                ...
                SystemAPI.Query<RefRO<FactionTag>, DynamicBuffer<TrainQueueItem>>())
            {
                ...
                for (int i = 0; i < trainQueue.Length; i++)
                {
                    ...
                    // Check if this is a builder unit type
                    if (item.UnitId=="Builder")
                    {
                        count++;
                    }
                }
            ...
            ref AIBuildingState buildingState, EntityCommandBuffer ecb)
        {
            ...
            // missing builders in queue!

            // Count builders already queued for training
            int queuedBuilders = CountQueuedBuilders(ref state, faction);
            int totalBuilders = builderCount + queuedBuilders;

            //UnityEngine.Debug.Log($"[AI Building] Faction {faction}: Active={builderCount}, Queued={queuedBuilders}, Total={totalBuilders}, Desired={buildingState.DesiredBuilders}");

            // Request more builders if needed (accounting for queued ones)
            ...
            DynamicBuffer<BuildRequest> buildReqs, EntityCommandBuffer ecb)
        {
            ...
            // Clean up completed requests whose builder died
        ...
        Entity builder, EntityCommandBuffer ecb)
    {
        ...
        // ============================================================
        // FIX: Deduct resources FIRST, before creating building entity
        // This prevents multiple buildings from spawning before resources update
        // ============================================================
        DeductBuildingCost(ref state, faction, req.BuildingType);

        // Now create the building entity AFTER resources are deducted
    ...
    private void DeductBuildingCost(ref SystemState state, Faction faction, FixedString64Bytes buildingType)
    {
        ...
        {
            // Find the AI brain for this faction to push recruitment requests into
